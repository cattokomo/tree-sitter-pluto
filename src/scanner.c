...
void tree_sitter_pluto_external_scanner_destroy(void *payload) {}
...
unsigned tree_sitter_pluto_external_scanner_serialize(void *payload, char *buffer) {
  buffer[0] = ending_char;
  buffer[1] = level_count;
  return 2;
}

void tree_sitter_pluto_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  if (length == 0) return;
...
bool tree_sitter_pluto_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
